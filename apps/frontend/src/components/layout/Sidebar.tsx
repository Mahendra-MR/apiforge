...
import { FolderClosed, History as HistoryIcon, Layers } from "lucide-react";
...
const TABS: { id: SidebarTab; label: string; icon: ReactNode }[] = [
  ...
  { id: "environments", label: "Environments", icon: <Layers size={17} strokeWidth={1.75} /> },
...
export function Sidebar() {
  ...
  return (
    <aside className="flex h-full w-full border-r border-slate-200 bg-white dark:border-white/[0.06] dark:bg-surface-dark-subtle">
      <nav
        aria-label="Sidebar"
        className="flex w-[78px] shrink-0 flex-col items-center gap-1 border-r border-slate-100 py-2 dark:border-white/[0.04]"
      >
        {TABS.map((item) => (
          <button
            key={item.id}
            ...
            className={clsx(
              "flex w-[70px] flex-col items-center gap-1 rounded-lg py-2 text-[10.5px] font-medium transition-colors",
              tab === item.id
